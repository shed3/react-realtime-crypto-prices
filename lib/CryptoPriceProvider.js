...
import { COIN_CAP_WS_URI, COIN_CAP_URI, CRYPTO_COMPARE_API_KEY, } from "./config";
...
export default function CryptoPriceProvider(_a) {
    ...
    var connectTickers = function (symbols) {
        state.tickersWebSocket = new WebSocket("wss://streamer.cryptocompare.com/v2?apiKey=" + CRYPTO_COMPARE_API_KEY);
            ...
            if (data.TYPE === "24" &&
                ...
                if (Object.keys(tickersCopy).includes(symbol)) {
                    if (state.tickers[symbol].timestamp <
                        ...
                        console.log("ME 1", formattedTicker);
                        dispatch({
                            type: STREAM_TICKER,
                            payload: formattedTicker,
                        });
                    }
                }
                else {
                    ...
                    console.log("ME 2", formattedTicker);
                    dispatch({
                        type: STREAM_TICKER,
                        payload: (_a = {},
